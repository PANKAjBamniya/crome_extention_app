...
import { ArrowLeft, Shield, Copy, Check, EyeOff, AlertTriangle, Clock, Info } from 'lucide-react'
...
export const SecretRecoveryPhrasePage: React.FC<SecretRecoveryPhrasePageProps> = ({
    account,
    seedPhrase,
    hasSrp,
    onBack,
        ...
        <div className="fixed inset-0 z-40 flex h-full w-full flex-col bg-black text-[#FCFAF9]">
            ...
            <div className="relative mx-auto flex h-full w-full max-w-md flex-col">
                ...
                <main className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                    ...
                    {!hasSrp ? (
                        /* Not available for imported account */
                        <div className="rounded-2xl border border-[#5E5E5E]/20 bg-black p-5 text-center space-y-3">
                            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-[#5E5E5E]/10 text-[#48E5C2]">
                                <Info size={24} />
                            </div>
                            <h3 className="text-sm font-semibold text-[#FCFAF9]">
                                Recovery Phrase Unavailable
                            </h3>
                            <p className="text-xs leading-5 text-[#5E5E5E]">
                                Secret Recovery Phrase is not available for this imported account. This account was imported directly using its private key.
                            </p>
                            <button
                                type="button"
                                onClick={onBack}
                                className="mt-3 rounded-xl bg-[#5E5E5E]/20 px-4 py-2 text-xs font-semibold text-[#FCFAF9] hover:bg-[#5E5E5E]/30 transition-colors"
                            >
                        ...
                        <>
                            ...
                            <div className="flex items-start gap-2.5 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-3.5 text-amber-300">
                                ...
                                <div className="space-y-1 text-xs">
                                    ...
                                    <p className="leading-5 text-amber-200/90 text-[11px]">
                                        These 12 words give complete access to your wallet and all assets across all accounts. Store them in a safe place offline.
                                    </p>
                                </div>
                            </div>

                            {/* 12-Word Recovery Grid */}
                            <div className="rounded-2xl border border-[#5E5E5E]/20 bg-black p-4">
                                ...
                                <div className="mt-3 grid grid-cols-3 gap-2.5 rounded-xl border border-[#5E5E5E]/20 bg-black p-3.5">
                                    {words.map((word, index) => (
                                        <div
                                            key={index}
                                            className="flex items-center gap-2 rounded-lg border border-[#5E5E5E]/20 bg-black px-2.5 py-2"
                                        >
                            ...
                            <div className="pt-2 space-y-2.5">
                                ...
                                >
                                    {copied ? (
                                        <>
                                            <Check size={16} />
                                            <span>Copied Phrase to Clipboard</span>
                                        </>
                                    ) : (
                                        <>
                                            <Copy size={16} />
